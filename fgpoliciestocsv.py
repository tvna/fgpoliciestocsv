...
option_1 = {
    'name': ('-o', '--output-file'),
    'help': '<OUTPUT_FILE>: output csv file (default \'./policies-out.csv\')',
    'default': 'policies-out.csv',
    'nargs': 1
}

option_2 = {
    'name': ('-n', '--newline'),
    'help': '<NEWLINE> : insert a newline between each policy for better readability',
    'action': 'store_true',
    'default': False
}
...
def parse(fd):
    ...
    @rtype:	return a list of policies
        ([
    ...
    """
    global p_entering_policy_block, p_exiting_policy_block, p_policy_next
    global p_policy_number, p_policy_set
        ...
        for line in fd_input:
            ...
            if in_policy_block:
                if p_policy_number.search(line):
                    policy_number = p_policy_number.search(
                        line).group('policy_number')
                ...
                if p_policy_set.search(line):
                    ...
                    policy_value = p_policy_set.search(
                        line).group('policy_value').strip()
