...
option_1 = {
    'name': ('-o', '--output-file'),
    'help': '<OUTPUT_FILE>: output csv file (default \'./groups-out.csv\')',
    'default': 'groups-out.csv', 'nargs': 1
}

option_2 = {
    'name': ('-n', '--newline'),
    'help': '<NEWLINE> : insert a newline between each group for better readability',
    'action': 'store_true',
    'default': False
}
...
p_entering_group_block = re.compile(
    '^\s*config firewall addrgrp$', re.IGNORECASE)
...
# -- Policy setting
p_group_set = re.compile(
    '^\s*set\s+(?P<group_key>\S+)\s+(?P<group_value>.*)$', re.IGNORECASE)
...
def parse(fd):
    ...
    @rtype:	return a list of groups ([
            {'id' : '1', 'srcintf' : 'internal', ...},
            {'id' : '2', 'srcintf' : 'external', ...},...])
            and the list of unique seen keys ['id', 'srcintf', 'dstintf', ...]
    """
    global p_entering_group_block, p_exiting_group_block, p_group_next
    global p_group_name, p_group_set
        ...
        for line in fd_input:
            ...
            if in_group_block:
                ...
                if p_group_set.search(line):
                    ...
                    group_value = p_group_set.search(
                        line).group('group_value').strip()
